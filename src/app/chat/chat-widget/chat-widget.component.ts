...
export class ChatWidgetComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private activatedRoute: ActivatedRoute,  public dialog: MatDialog,
    private router: Router, private dom: DomSanitizer, private service: ChatService,  private dataService: DataService, private toastrService: ToastrService,
    private headerComponent: ChatHeaderComponent) {
  ...
  public extract_links(raw_message) {
    ...
    if (typeof raw_message == 'object') {
      if (typeof raw_message['buttons'] == 'object') {
        ...
        if (jsn instanceof Array == false && (jsn['isTokenRefreshed'] === false || jsn['isTokenRefreshed'] === true)) {
        ...
        if(jsn[0]["form_type"] == "feedback"){
          return this.updateFeedback();
        }else if(jsn[0]["forms_type"] == "toast"){
          return this.displayToast(jsn);
        }else if(jsn[0]["forms_type"] == "theme"){
          return this.themeSelection(jsn[0]['colour']);
        }else if(jsn[0]["forms_type"] == "timestamp"){
          return this.updateTimestamp(jsn);
        }else if(jsn[0]["forms_type"] == "checkbox"){
        ...
        }else if(jsn[0]["forms_type"]  == "arrowmark"){
          jsn.shift();
          for(var l=0;l<jsn.length;l++){
            jsn[l]['title'] = this.linkCreationURL(jsn[l]['title']);
          }
          arrowmark = jsn;
        }else if(jsn[0]["forms_type"]  == "news"){
          news = jsn;
        }else if(jsn[0]["forms_type"] == "conversation"){
    ...
    if (link.length > 1 && tabs.length == 0) {
      // let lk_text = link[0].split('[')
      // let link_url = link[1].split(')');
      // // let link_data = '<a _ngcontent-c0 href=' + link_url[0] + ' rel="noopener noreferrer" target="_blank">' + lk_text[1] + '</a>'
      // urlLinkData = {
      //   preText: lk_text[0],
      //   urlLink: link_url[0],
      //   textLink: lk_text[1],
  ...
  /**
    * @description flight click event
  */
  ...
  public addMessage(from, r_text, type: 'received' | 'sent', isDisplay = true, isUnshift=true) {
    ...
    if (isDisplay == true) {
      ...
      // let uimessage = this.handlinguimessages(validatedText, from, r_text, type, isDisplay)
      // this.scrollToBottom()
  ...
  public scrollToBottom() {
    if (this.bottom !== undefined) {
      ...
      setTimeout(function () {
        ...
        that.bottom.nativeElement.scrollTop = that.bottom.nativeElement.scrollHeight;
      }, 100)
    }
  }
  ...
  public handleResponseMessages(data, isDisplay = true) {
    ...
    if (data[0] == undefined && isDisplay) {
      ...
      // this.addMessage(this.operator, this.defaultMsg, 'received', isDisplay)
    }
  ...
  ngOnInit() {
    ...
    this.dataService.currentTheme.subscribe(message => {
      if(this.initialSubscribe){
      this.themeSelectionEvent(message)}
      this.initialSubscribe = true;
    })
    // if (authDetails[0] != undefined) {
  ...
  public sendMessage({ message }, isDisplay = true, extDetails = {}, isActionDisplay = true) {
    ...
    // setTimeout(() => this.randomMessage(message, isDisplay), 1000)
  }
  likeButton(bol){
  ...
  conversationEvents(){
    ...
    // this.sendMessage_custom('/Events', undefined, undefined, false)
  }
  ...
  themeSelection(val){
    let themeVal = [{type:'blue_blue', colour:"blue", disabled:false, selectedClass : "blue_blue_button tgl_btn"},
    ...
    // this.headerComponent.themeSelection(this.theme, this.toggleTheme)
    this.headerComponentTheme()
  }

  headerComponentTheme() {
    this.dataService.changeToggleTheme(this.toggleTheme);
  }
...
// handleAuthorizationKeys(data) {
// }
// // sucess updating token
